const { radToDeg, degToRad, getJulianDate } = require("./utils");
...
function getMoonPosition(time = new Date()) {
	...
	const JDE = 2448724.5; //julian date
	...
	const apparentLongitude = longitude + 0.00461; //nutation in longitude (chapter 22)
	const latitude = sumOfLatitudes / 10 ** 6; //degrees
	const distance = 385000.56 + sumofDistances / 10 ** 3; //kilometers

	console.log(`${apparentLongitude}, ${latitude}`);
	return { latitude, longitude: apparentLongitude, distance };
}
